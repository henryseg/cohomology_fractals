...
float amountOutsideTetrahedron(vec4 v, out int biggest_face) {
...
float graph_trace(vec4 pixel_pt, int tetNum){
  ...
  int index;
  int biggest_face;
  float amount_outside = amountOutsideTetrahedron(pixel_pt, biggest_face);
  for(int i=0; i<maxSteps; i++){
      if ( amount_outside > 0.000001 && biggest_face != entry_face ){
        ...
        amount_outside = amountOutsideTetrahedron(pixel_pt, biggest_face);
      }
      else{
        break;
      }
...
void main(){
  ...
  vec4 pixel_pt = pointOnGeodesic(init_pt, init_dir, maxDist);

  float weight = currentWeight + graph_trace(pixel_pt, tetNum);
  weight = 0.3 * weight;
